...
import { IconLayers, IconMail, IconLock, IconCheck, IconShield } from "../utils/helpers";
...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // ✅ With Promise Toast
      const result = await showPromise(dispatch(loginUser(formData)).unwrap(), {
        loading: "Logging in...",
      ...
      <div className="relative hidden overflow-hidden border-r border-[#232A38] bg-gradient-to-br from-[#131720] to-[#0F131B] lg:flex lg:flex-col lg:justify-between lg:p-12">
        ...
        <div className="relative max-w-sm">
          <h2 className={`${FONT_DISPLAY} text-3xl font-semibold leading-[1.15] tracking-tight text-[#E8EAED]`}>
            Stock that <span className="text-[#34D1BF]">never lies</span>, no matter how busy it gets.
          </h2>
          <ul className="mt-8 space-y-3.5">
            {["Role-based access for your whole team", "Real-time stock, protected under load", "A complete audit trail on every change"].map(
              (item) => (
      ...
      <div className="flex min-h-screen items-center justify-center px-5 py-12 sm:px-8">
        <div className="w-full max-w-sm">
          ...
          <p className="mt-2 text-center text-sm text-[#8B93A1] sm:text-left">
            New to StockXpress?{" "}
            <Link to="/register" className="font-medium text-[#FF6B1A] hover:text-[#FF7A30]">
              Create an account
            </Link>
          </p>
